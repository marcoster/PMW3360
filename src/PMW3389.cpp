...
void PMW3389::setCPI(unsigned int cpi)
{
  ...
  adns_write_reg(REG_Resolution_L, (uint8_t)(cpival & 0xFF));
  adns_write_reg(REG_Resolution_H, (uint8_t)((cpival >> 8) & 0xFF));
  SPI_END;
}
...
{
    return readBurst(PMW3389_BURST_DATA_SIZE);
}
...
type: PMW3389_DATA
*/
PMW3389_DATA PMW3389::readBurst(uint8_t num_read)
{  
  unsigned long fromLast = micros() - _lastBurst;
  PMW3389_DATA data;

  if(num_read > PMW3389_BURST_DATA_SIZE) {
      num_read = PMW3389_BURST_DATA_SIZE;
  }
  num_read = constrain(num_read, PMW3389_BURST_DATA_MIN_SIZE, PMW3389_BURST_DATA_SIZE);
